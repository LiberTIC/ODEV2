...
class CalendarOld extends AbstractBackend implements SyncSupport, SubscriptionSupport, SchedulingSupport
{
    ...
    public function getCalendarsForUser($principalUri)
    {
        ...
        foreach ($searchResult as $cal) {
            ...
            $calendar = array(  
                      ...
                      '{'.CalDAV\Plugin::NS_CALENDARSERVER.'}getctag' => 'http://sabre.io/ns/sync/'.$src['synctoken'],
                      '{http://sabredav.org/ns}sync-token' => $src['synctoken'],
                      '{'.CalDAV\Plugin::NS_CALDAV.'}supported-calendar-component-set' => new CalDAV\Xml\Property\SupportedCalendarComponentSet($src['components']),
                      '{'.CalDAV\Plugin::NS_CALDAV.'}schedule-calendar-transp' => new CalDAV\Xml\Property\ScheduleCalendarTransp($src['transparent'] ? 'transparent' : 'opaque'),
                      '{DAV:}displayname' => $src['displayname'],
                      '{urn:ietf:params:xml:ns:caldav}calendar-description' => $src['description'],
                      '{urn:ietf:params:xml:ns:caldav}calendar-timezone' => $src['timezone'],
                      '{http://apple.com/ns/ical/}calendar-order' => $src['calendarorder'],
